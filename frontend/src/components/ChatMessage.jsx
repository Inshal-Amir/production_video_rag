  ...
  return (
    <div className="message-row bot" style={{ alignItems: "flex-start" }}>
      ...
      <div className="bot-response-container">
            ...
            <table className="custom-table">
              ...
              <tbody>
                {topResults.map((result, idx) => {
                  const aiTextStr = (message.text || "").trim().toLowerCase();
                  const isAiPositive =
                    aiTextStr.startsWith("yes") ||
                    (aiTextStr.includes("visible") &&
                      !aiTextStr.includes("not visible"));

                  // Map "Found" to Yes if LLM confirmed (at least for the top result) OR if score is high
                  const isMatch =
                    (isAiPositive && idx === 0) ||
        ...
        {/* 3. SUMMARY TEXT BOX */}
        <div className="summary-box">{message.text}</div>
      </div>
    </div>
  );
};
